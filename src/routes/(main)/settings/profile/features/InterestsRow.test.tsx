  ...
  useTranslation: (namespace: string) => ({
    t: (key: string) => {
      ...
      return (
        (
          {
            'interests.area.english': '英语',
            'interests.area.highSchool': '高中',
            'interests.area.math': '数学',
            'interests.area.middleSchool': '初中',
            'interests.area.other': '其他领域',
            'interests.area.physics': '物理',
            'interests.area.primarySchool': '小学',
...
describe('InterestsRow', () => {
  it('stores predefined interests as canonical keys', async () => {
    ...
    await waitFor(() => {
      expect(mocks.updateInterests).toHaveBeenCalledWith(['math']);
    });
  });

  it('removes predefined interests by canonical key', async () => {
    const user = userEvent.setup();
    mocks.interests = ['math', '自定义'];
