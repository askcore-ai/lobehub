...
const AskAgentCommands = memo(() => {
  ...
  // Check if AskCore AI matches the query
  const showAskCoreAI = !mentionQuery || 'askcore ai'.includes(mentionQuery);
    ...
    <Command.Group heading={t('cmdk.mentionAgent')}>
      ...
      {showAskCoreAI && (
        <Command.Item
          ...
          onSelect={() => handleAgentSelect(inboxAgentId, 'AskCore AI', DEFAULT_INBOX_AVATAR)}
        >
          <Avatar emojiScaleWithBackground avatar={DEFAULT_INBOX_AVATAR} shape="square" size={18} />
          <div className={styles.itemContent}>
            <div className={styles.itemLabel}>@AskCore AI</div>
          </div>
        </Command.Item>
      )}
