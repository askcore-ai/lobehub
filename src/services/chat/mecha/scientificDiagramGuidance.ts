...
export const SCIENTIFIC_DIAGRAM_OUTPUT_GUIDANCE =
  ...
  '- Valid ASCII-only body example: `\\begin{tikzpicture} \\draw (0,0) -- (2,0) node[right] {Output}; \\end{tikzpicture}`. Wrap that body in the required `tikz` fence; do not add comments or localized labels.\n' +
  ...
  '- FINAL MANDATORY VALIDATION: Before sending, inspect every character between each `tikz` fence\'s opening and closing backticks. If any character is outside U+0000-U+007F, remove all comments and rewrite every label in ASCII. Never send the response until this check passes.\n' +
  '</scientific_diagram_output_guidance>';
