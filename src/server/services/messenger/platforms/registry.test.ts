...
describe('messengerPlatformRegistry singleton', () => {
  it('registers slack, telegram, and discord on import', async () => {
    vi.resetModules();
    vi.doMock('./slack', () => ({
      MessengerSlackBinder: class {},
      slack: buildDefinition({ id: 'slack', name: 'Slack' }),
      slackWebhookGate: {},
    }));
    vi.doMock('./telegram', () => ({
      MessengerTelegramBinder: class {},
      telegram: buildDefinition({ id: 'telegram', name: 'Telegram' }),
    }));
    vi.doMock('./discord', () => ({
      MessengerDiscordBinder: class {},
      buildDiscordSwitchButtons: vi.fn(),
      discord: buildDefinition({ id: 'discord', name: 'Discord' }),
    }));
