...
export type AskCoreProtocolRouteMode = 'identity-link' | 'processing' | 'registration';
...
export const askCoreProtocolRouteMode = (search: string): AskCoreProtocolRouteMode | null => {
  ...
  return mode === 'processing' || mode === 'identity-link' || mode === 'registration' ? mode : null;
};
...
export const AskCoreProtocolRoute = memo(() => {
  ...
  if (mode === 'registration') return <RegistrationProvisioningSurface />;
  if (mode === 'processing') {
