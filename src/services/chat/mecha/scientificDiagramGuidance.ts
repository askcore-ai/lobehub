...
export const SCIENTIFIC_DIAGRAM_OUTPUT_GUIDANCE =
  ...
  '- For a generic circuit request that does not specify a topology or components, copy the following complete ASCII-only body exactly and do not add, remove, translate, or modify any fence-body character: `\\begin{tikzpicture} \\draw (0,0) to[battery1,l=$V_s$] (0,3) -- (3,3) to[R,l=$R$] (3,0) -- (0,0); \\end{tikzpicture}`. Wrap it in the required `tikz` fence.\n' +
  '- 中文强制规则：即使用户使用中文提问，`tikz` 围栏内部也绝不能出现中文、全角标点或任何非 ASCII 字符。\n' +
  '- 若用户没有指定电路拓扑或元件，必须逐字复制上一条给出的固定电路模板，禁止添加“输入”“输出”“电源”等中文标签；中文解释只能写在围栏外。\n' +
...
const SCIENTIFIC_DIAGRAM_GUIDANCE_SCOPES: ReadonlySet<string> = new Set<string>([
  'group',
  'group_agent',
  'main',
  'thread',
